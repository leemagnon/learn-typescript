...
// DOM - Element, HTMLElement, HTMLParagraphElement
...
fetchCovidSummary().then(res => res.data.Global.NewRecovered);
...
function setTotalDeathsByCountry(data: any) {
  deathsTotal.innerText = data[0].Cases; // Element 타입에 innerText가 없는 이유.
}
...
function setChartData(data: any) {
  ...
  const chartLabel = data
    .slice(-14)
    .map((value: any) =>
      new Date(value.Date).toLocaleDateString('ko-KR').slice(5, -1)
    );
